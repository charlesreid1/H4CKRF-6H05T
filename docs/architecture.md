...
|------|---------|----------|
| **LOW** | Read-only, no TX, bounded duration. Executed immediately. | `get_device_info`, `sweep_spectrum` (dwell ≤ 2 s), `capture_iq` (duration ≤ 5 s), `grant_list`, `audit_query`, `read_iq_summary`, `analyze_pulses`, `demodulate_bits` |
