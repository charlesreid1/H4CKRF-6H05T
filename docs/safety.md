...
|------|---------|----------|----------|
| **LOW** | Auto-execute, no confirmation | Read-only, bounded duration, no disk write outside session dir | `get_device_info`, `sweep_spectrum` (RX, ≤2 s dwell), short `capture_iq` (≤5 s, to session path), `grant_list`, `audit_query`, `analyze_pulses`, `demodulate_bits` |
| **MEDIUM** | Single-key confirmation (Y/n) | Longer capture, higher gain, disk writes, TX in ISM bands within legal power | `capture_iq` >5 s, `transmit_iq` on 433.05–434.79 MHz with gain ≤ 30 dB |
